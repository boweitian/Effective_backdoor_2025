...
from models import *
from utils import optimizer_picker, pth_generator, load_model_state, load_model_state_vis
...
def main():
    ...
    parser.add_argument('-task', type=str, required=True, help='Specfiy the task ('
                                                               'vggflower/cifar10/gtsrb/cifar100/imagenette/vgg224).')
    ...
    parser.add_argument('-iter', type=int, default=1, help='Specfiy the task (vggflower/cifar10/gtsrb/cifar100).')
    parser.add_argument('-b', type=int, default=16, help='Batch Size for model retrain')
    parser.add_argument('-nw', type=int, default=16, help='Num_worker for pytorch dataloader')
    parser.add_argument('-lr', type=float, default= 5e-4, help='learning rate for model retrain')
    parser.add_argument('-e', type=int, default=30, help='the num of epoches for a single retrain round')
    parser.add_argument('-tc', type=str, default='', help='target class of this time, you can input several')
    ...
    file_pth, log_path, ckpt_path, file_stream = pth_generator(dataset_nm, time.time(), mask_mode, alternate, multi_neuron, invisible_train,
                                                  poison_rate,
                                                  pixel_nums, mask_opacity, multi=True)
    ...
    model = load_model_state_vis(model, pretrained_model_pth, device)
    ...
    if args.tc == '':
        ...
        tc = [int(i) for i in args.tc.split(',')]
    trigger = torch.from_numpy(trigger)
    trigger = trigger.float()
    if normalize != None:
        trigger = normalize(trigger)
    masks = []
    for target_class in tc:
        masks.append(helper.filter_apart(trainset_ori + testset, target_class))
    if hasattr(helper, 'RAN'):
        del helper.RAN
    # print(masks, file=file_stream)
    ...
    while helper.iter_counter == 0 or helper.not_converge_test():
        ...
        for index, mask in enumerate(masks):
            ...
            target_class = tc[index]
            triggers[index] = helper.gen_trigger(model, trigger, mask, target_class, transform[1], trainset_ori,
                                           inv_normalize)
            need_original = True if index == 0 else False
            trainset_poisoned.extend(helper.poison_v2(target_class, trainset_ori,
                                                    triggers[index], mask, w, h, transform[0],
                                                    train=True, inv_normalize=inv_normalize, need_original=need_original))
            gc.collect()
            testset_poisoned.extend(helper.poison_v2(target_class,  testset,
                                                     triggers[index], mask, w, h, transform[1],
                                                     train=False, inv_normalize=inv_normalize))
            gc.collect()
        # the x_poison have been transformed after the poison function
        # _initialize_weights(model)
        print(len(trainset_poisoned))
        print(len(testset_poisoned))
        ...
        for e in range(helper.rt_epoches):
            ...
            helper.evaluation(loss_list, model, data_loader_test_ori, data_loader_test_p, triggers, masks,
                                       transform[1], e)
            end_time = time.time()
        ...
        helper.evaluation(['fidelity'], model, data_loader_test_ori, data_loader_test_p, triggers, masks,
                          transform[1], 0)
